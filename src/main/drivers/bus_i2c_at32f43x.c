 ...
 *
 * 尝试模仿stm32 直接读取i2c 寄存器方式实现i2c，发现状态对应很困难
 * 暂时放弃此文件， 改用模仿HAL 方式，直接使用at32 bsp 提供的样例进行移植
 * Warning: not use this file,use bus_i2c_atbsp_init bus_i2c_atbsp INSTEAD
 */
...
/*stm32 与at32f43x
* I2C_IT_EVT 和 I2C_IT_ERR 的对应
* at32 RM 11.5  vs   stm32f4 UM 18.4
* I2C_IT_EVT = I2C_TD_INT | I2C_RD_INT| I2C_ADDR_INT |I2C_STOP_INT |I2C_ACKFAIL_INT |I2C_ADDR_INT |I2C_TDC_INT
* I2C_IT_ERR = I2C_ERR_INT
*/
#define I2C_IT_EVT  (I2C_TD_INT | I2C_RD_INT| I2C_ADDR_INT |I2C_STOP_INT |I2C_ACKFAIL_INT |I2C_ADDR_INT |I2C_TDC_INT)
#define I2C_IT_ERR  (I2C_ERR_INT )
...
bool i2cWriteBuffer(I2CDevice device, uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
    ...
    i2c_type *I2Cx = i2cDevice[device].reg;
...
//        I2C_ITConfig(I2Cx, I2C_IT_EVT | I2C_IT_ERR, ENABLE);            // allow the interrupts to fire off again
        i2c_interrupt_enable(I2Cx,I2C_IT_EVT | I2C_IT_ERR, TRUE);
    }
...
bool i2cReadBuffer(I2CDevice device, uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t* buf)
{
    ...
    state->busy = 1;
    state->error = false;
//fixme: modified should be start here !
    if (!(I2Cx->CR2 & I2C_IT_EVT)) {                                    // if we are restarting the driver
        if (!(I2Cx->CR1 & I2C_CR1_START)) {                             // ensure sending a start
            while (I2Cx->CR1 & I2C_CR1_STOP) {                          // wait for any stop to finish sending
                if (cmpTimeUs(microsISR(), timeoutStartUs) >= I2C_TIMEOUT_US) {
...
static void i2c_er_handler(I2CDevice device) {
    ...
    i2c_type *I2Cx = i2cDevice[device].hardware->reg;

    i2cState_t *state = &i2cDevice[device].state;

    // Read the I2C1 status register
//    volatile uint32_t SR1Register = I2Cx->SR1;
    volatile uint32_t SR1Register = I2Cx->sts;

/* stm32 i2c error mapping to at32
 * 错误处理部分是可以一一映射的，流程不做修改
  ...
  *I2C_SR1_AF       - I2C_ACKFAIL_FLAG: acknowledge failure flag.
  ...
  *I2C_SR1_BERR        - I2C_BUSERR_FLAG: bus error flag.
  *I2C_SR1_ARLO         - I2C_ARLOST_FLAG: arbitration lost flag.
  *I2C_SR1_OVR         - I2C_OUF_FLAG: overflow or underflow flag.
...
//    if (SR1Register & (I2C_SR1_BERR | I2C_SR1_ARLO | I2C_SR1_AF | I2C_SR1_OVR)) // an error
    if (SR1Register & (I2C_BUSERR_FLAG | I2C_ARLOST_FLAG | I2C_ACKFAIL_FLAG | I2C_OUF_FLAG))
        state->error = true;

    // If AF, BERR or ARLO, abandon the current job and commence new if there are jobs
    if (SR1Register & (I2C_BUSERR_FLAG | I2C_ARLOST_FLAG | I2C_ACKFAIL_FLAG)) {
//        (void)I2Cx->SR2;                                                        // read second status register to clear ADDR if it is set (note that BTF will not be set after a NACK)
    	I2Cx->clr_bit->addrc=1;

//        I2C_ITConfig(I2Cx, I2C_IT_BUF, DISABLE);                                // disable the RXNE/TXE interrupt - prevent the ISR tailchaining onto the ER (hopefully)
    	i2c_interrupt_enable(I2Cx,I2C_RD_INT | I2C_TD_INT, FALSE);

        if (!(SR1Register & I2C_ARLOST_FLAG) && !(1 & I2Cx->ctrl2_bit.genstop)) {     // if we dont have an ARLO error, ensure sending of a stop
            if (I2Cx->ctrl2_bit.genstart == 1) {                                    // We are currently trying to send a start, this is very bad as start, stop will hang the peripheral
                while (I2Cx->ctrl2_bit.genstart == 1) {; }                         // wait for any start to finish sending
//                I2C_GenerateSTOP(I2Cx, ENABLE);                                 // send stop to finalise bus transaction
                I2Cx->ctrl2_bit.genstop=1;
                while (I2Cx->sts.stopf == 0 ) {; }                               // wait for stop to finish sending
                I2Cx->clr.stopc=1;
...
//                I2C_ITConfig(I2Cx, I2C_IT_EVT | I2C_IT_ERR, DISABLE);           // Disable EVT and ERR interrupts while bus inactive
            	i2c_interrupt_enable(I2Cx,I2C_IT_EVT | I2C_IT_ERR, FALSE);
            }
        }
    }
//    I2Cx->SR1 &= ~(I2C_SR1_BERR | I2C_SR1_ARLO | I2C_SR1_AF | I2C_SR1_OVR);     // reset all the error bits to clear the interrupt
    i2c_flag_clear(I2Cx,I2C_BUSERR_FLAG | I2C_ARLOST_FLAG | I2C_ACKFAIL_FLAG | I2C_OUF_FLAG)

    state->busy = 0;
}

/*I2C 事件处理
 * 这部分  at32 无法和stm32 完全1：1 对应，需要根据at32 的硬件来进行修改
 * 1、缺少startBit send 事件
 * 2、多ackfail事件
 */
...
void i2cInit(I2CDevice device)
{
    ...
    i2c_type  *I2Cx = hw->reg;

    memset(&pDev->state, 0, sizeof(pDev->state));

//    NVIC_InitTypeDef nvic; //nvic 直接写
//    I2C_InitTypeDef i2cInit; //init 拆解为多个函数，不需要
...
//    i2cInit.I2C_Mode = I2C_Mode_I2C; //i2c ,not smbus
//    i2cInit.I2C_DutyCycle = I2C_DutyCycle_2;   // 需要映射到 i2c clkctrl 寄存器  sdaD SCLD sclH sclL
//    i2cInit.I2C_OwnAddress1 = 0; //own_address1_set
//    i2cInit.I2C_Ack = I2C_Ack_Enable;//ack enable
//    i2cInit.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit; //own_address1_set
//    i2cInit.I2C_ClockSpeed = pDev->clockSpeed * 1000; // 总线频率，需要和duty_cycle 一起映射到clkctrl.div
//    RM 11.4.1 I2c时序计算 各参数关系 11.7.5 配置参数表

//    I2C_Cmd(I2Cx, ENABLE);
//    I2C_Init(I2Cx, &i2cInit);
    //calc clkctrl
    //用 pDev->clockSpeed * 1000 + I2C_DUTYCYCLE_2 默认配置拼出clkctrl 寄存器值
    //init i2c
    i2c_init(I2Cx,0,);//i2c,dfilter, clk,clk => clkctrl 寄存器，需要用i2c 配置工具生成
    //enable i2c
    i2c_enable(I2Cx,TRUE);//i2c_init ctrl1_i2cen =0 ,so enable
...
//    I2C_StretchClockCmd(I2Cx, ENABLE);//允许时钟延展
